            ...
            <div className="stats-grid stagger">
                <div className="card stat-card">
                    ...
                    <div className="stat-content">
                        <span className="text-muted font-bold text-xs uppercase tracking-wider mb-1">Planes Activos</span>
                        <span className="stat-value">{financiaciones.filter(f => f.estado === 'activa').length}</span>
                    </div>
                </div>
                ...
                <div className="card stat-card">
                    ...
                    <div className="stat-content">
                        <span className="text-muted font-bold text-xs uppercase tracking-wider mb-1">Alerta de Mora</span>
                        <span className="stat-value">{financiaciones.filter(f => f.estado === 'en_mora').length}</span>
                    </div>
                </div>
                ...
                <div className="card stat-card">
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <Badge variant="success">Recobrable</Badge>
                    </div>
                    <div className="stat-content">
                        <span className="text-muted font-bold text-xs uppercase tracking-wider mb-1">Cartera Administrada</span>
                        <span className="stat-value">${financiaciones.reduce((s, f) => s + Number(f.montoFinanciado), 0).toLocaleString('es-AR')}</span>
                    </div>
                </div>
            </div>
            ...
            >
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
                    <div className="input-group">
                        <label className="input-label">Venta de Origen (Documento Base) *</label>
                        <select className="input-control" value={form.ventaId || ''} onChange={e => setForm(f => ({ ...f, ventaId: +e.target.value }))}>
                            <option value="">Seleccioná un contrato de transferencia…</option>
                            {ventas.map(v => (
                                <option key={v.id} value={v.id}>
                                    {`#${String(v.id).padStart(5, '0')} — ${v.cliente?.nombre} — ${v.vehiculo?.marca} ${v.vehiculo?.modelo}`}
                                </option>
                            ))}
                        </select>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
                        <div className="input-group">
                            <label className="input-label">Titular exigible *</label>
                        ...
                        <div className="input-group">
                            <label className="input-label">Oficial responsable</label>
                            <select className="input-control" value={form.cobradorId || ''} onChange={e => setForm(f => ({ ...f, cobradorId: +e.target.value }))}>
                                <option value="">Asigná un gestor recaudador…</option>
                                {cobradores.map(u => <option key={u.id} value={u.id}>{u.nombre}</option>)}
                            </select>
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 'var(--space-4)' }}>
                        <div className="input-group">
                            <label className="input-label">Capital liquidado *</label>
                        ...
                        <div className="input-group">
                            ...
                            <select className="input-control" value={form.cuotas} onChange={e => setForm(f => ({ ...f, cuotas: +e.target.value }))}>
                                {[1, 3, 6, 12, 18, 24, 36, 48, 60].map(n => <option key={n} value={n}>{n} cuotas mensuales</option>)}
                            </select>
                        </div>
                        <div className="input-group">
                            <label className="input-label">Interés mensual (%)</label>
                            <div className="input-container has-icon">
                                <span className="input-icon" aria-hidden="true" style={{ color: 'var(--accent-3)' }}><TrendingUp size={14} /></span>
                                <input type="number" step="0.1" className="input-control" placeholder="0.0"
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
                        <div className="input-group">
                            <label className="input-label">Fecha de inicio contable</label>
                            <input type="date" className="input-control" value={form.fechaInicio} onChange={e => setForm(f => ({ ...f, fechaInicio: e.target.value }))} />
                        </div>
                        <div className="input-group">
                            <label className="input-label">Día fijo de cobro</label>
                            <select className="input-control" value={form.diaVencimiento} onChange={e => setForm(f => ({ ...f, diaVencimiento: +e.target.value }))}>
                    ...
                    <div className="input-group">
                        <label className="input-label">Notas del contrato (observaciones)</label>
                        <textarea className="input-control" rows={2}
                            value={form.observaciones}
                            onChange={e => setForm(f => ({ ...f, observaciones: e.target.value }))}
                            placeholder="Especificá garantías o acuerdos excepcionales…"
                        ...
                        <div style={{
                            ...
                            <div>
                                ...
                                <p style={{
                                    ...
                                    ${calcMontoCuota().toLocaleString('es-AR')}
                                </p>
                            </div>
                            <div style={{ textAlign: 'right' }}>
                                ...
                                <p style={{
                                    ...
                                    ${Number(form.montoFinanciado).toLocaleString('es-AR')}
                                </p>
                            </div>
                        </div>
                    )}
            ...
            >
                {pagarCuota && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
                        <div style={{
                            ...
                            <div>
                                <span style={{
                                    ...
                                    Cuota #{pagarCuota.nroCuota}
                                </span>
                                <p style={{ fontSize: 'var(--text-xs)', color: 'var(--accent)', fontWeight: 600, margin: 0 }}>
                                    Fecha límite: {new Date(pagarCuota.vencimiento).toLocaleDateString('es-AR')}
                                </p>
                            </div>
                            <div style={{ textAlign: 'right' }}>
                                <span style={{
                                ...
                                <p style={{
                                    ...
                                    ${Number(pagarCuota.saldoCuota).toLocaleString('es-AR')}
                                </p>
                            </div>
                        </div>

                        <div className="input-group">
                            <label className="input-label">Recaudación efectiva (ARS) *</label>
                            <div className="input-container has-icon">
                                <span className="input-icon" aria-hidden="true" style={{ color: 'var(--accent)' }}><DollarSign size={14} /></span>
                                <input type="number" className="input-control" placeholder="0.00"
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
                            <div className="input-group">
                                <label className="input-label">Vía de ingreso *</label>
                                <select className="input-control" value={pagoForm.metodo} onChange={e => setPagoForm(f => ({ ...f, metodo: e.target.value as PagarCuotaDto['metodo'] }))}>
                                    {Object.entries(metodoLabels).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
                                </select>
                            </div>
                            <div className="input-group">
                        ...
                        <div className="input-group">
                            <label className="input-label">Referencia / tracking</label>
