...
const UsuariosPage: React.FC = () => {
    ...
    const canManageUsuarios = !!currentUser?.roles?.some(r => r === 'admin' || r === 'super_admin');
    ...
    const resendInviteMutation = useMutation({
        mutationFn: (id: number) => accountApi.resendInvitation(id),
        onSuccess: () => {
            addToast('Invitación reenviada por email.', 'success');
        },
        onError: (error) => {
            addToast(getApiErrorMessage(error, 'No se pudo reenviar la invitación'), 'error');
        },
    });
    ...
    return (
        <div className="page-container">
                                ...
                                <tr key={usuario.id}>
                                    ...
                                    <td>
                                        {usuario.estado === 'pendiente' ? (
                                            <span className="badge badge-warning" title="Esperando que el usuario active su cuenta">
                                                <Clock size={11} style={{ marginRight: 4 }} />
                                                Pendiente
                                            </span>
                                        ) : usuario.estado === 'bloqueado' ? (
                                            <span className="badge badge-danger">
                                                <Ban size={11} style={{ marginRight: 4 }} />
                                                Bloqueado
                                            </span>
                                        ) : !usuario.activo ? (
                                    ...
                                    <td>
                                        <div className="action-buttons">
                                            {canManageUsuarios && usuario.estado === 'pendiente' && (
                                                <button
                                                    onClick={() => resendInviteMutation.mutate(usuario.id)}
                                                    className="icon-btn"
                                            ...
                                            {canManageUsuarios && usuario.estado === 'activo' && (
                                                <button
            ...
            <style>{`
                ...
                .users-title svg { color: var(--accent); }
                .users-title h1 {
