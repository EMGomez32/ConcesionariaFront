...
const ConfiguracionPage = () => {
    ...
    const tabBtnStyle = (active: boolean): React.CSSProperties => ({
        ...
        background: active ? 'var(--accent-gradient, linear-gradient(135deg, #6366f1, #4f46e5))' : 'transparent',
        ...
        <div className="page-container animate-fade-in" style={{ maxWidth: '900px' }}>
            <header className="page-header" style={{ marginBottom: '1.5rem' }}>
                <div className="header-title">
                    ...
                    <p style={{ color: 'var(--text-secondary)', marginTop: '0.5rem' }}>Administrá tu concesionaria, tu perfil y la seguridad de tu cuenta.</p>
                </div>
            </header>

            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
                <button onClick={() => setTab('concesionaria')} style={tabBtnStyle(tab === 'concesionaria')}>
                    <Building2 size={16} /> Mi concesionaria
                </button>
                <button onClick={() => setTab('perfil')} style={tabBtnStyle(tab === 'perfil')}>
                    <UserIcon size={16} /> Mi perfil
                </button>
                <button onClick={() => setTab('password')} style={tabBtnStyle(tab === 'password')}>
                ...
                <div className="glass" style={{ padding: '1.5rem', borderRadius: '1rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                        <div>
                            ...
                            <input type="password" className="form-input" value={passForm.password}
                                onChange={e => setPassForm(f => ({ ...f, password: e.target.value }))}
                                placeholder="Mínimo 6 caracteres" />
                        </div>
                        <div>
                            <label style={labelStyle}>Confirmar contraseña *</label>
                            <input type="password" className="form-input" value={passForm.confirm}
